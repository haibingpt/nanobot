...
| 场景 | scope_dir 路径 | channel_name 来源 |
|------|----------------|-------------------|
| Discord guild channel | `discord/{channel_name}/` | Discord API `channel.name` |
| Discord thread | `discord/{parent_channel_name}/` | 递归取 parent channel name |
| Discord DM | `discord/dm-{user_name}/` | DM 对方用户名，加 `dm-` 前缀防冲突 |
| CLI 直聊 | `discord/cli/` | 硬编码 "cli" |

**设计决策：**
- CLI 模式不独立顶层目录，视为 discord 下的特殊 channel
- DM 加 `dm-` 前缀避免与 guild channel 同名冲突
- channel_name fallback：拿不到时用 chat_id 作目录名
- 不需要持久化 id→name 映射文件（用户不 rename channel，只新增）
- channel rename 策略：暂不考虑
